...
const formikSchema = Yup.object().shape({
    ...
    username: Yup.string()
        ...
        .min(10, 'User Name must be at least 3 numbers')
        .max(15, 'User Name must be 15 numbers'),
    email: Yup.string()
...
const Signup = () => {
    ...
    const loginUser = async (values) => {
        ...
        try {
            ...
            localStorage.setItem('userDetailes', JSON.stringify(result?.res));
            ...
            <div className=' z-[1] w-[80vw] h-[90vh] rounded-[10px] bg-white flex relative overflow-hidden'>
                ...
                <div className={`w-[65%] rounded-l-[10px] flex flex-col items-center justify-center h-full gap-[20px] absolute ${ani ? 'animate-formSecondReturn' : 'animate-formSecond'}`}>
                    ...
                    <div className='flex items-center w-full justify-center flex-col gap-[10px]'>
                        ...
                        <InputLoginComponent icon={<CiMobile1 />} placeholder={'User Name'} name={'username'} value={formikSignup?.values?.username} changeHandler={formikSignup.handleChange} errors={formikSignup?.errors?.username} touched={formikSignup?.touched?.username} />
